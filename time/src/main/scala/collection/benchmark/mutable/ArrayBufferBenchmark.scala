...
class ArrayBufferBenchmark extends AbstractBenchmark {
  ...
  val empty: ArrayBuffer[Long] = ArrayBuffer.empty[Long]
  var xs: ArrayBuffer[Long] = _
  var ys: ArrayBuffer[Long] = _
  var zs: ArrayBuffer[Long] = _
  var zipped: ArrayBuffer[(Long, Long)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = ArrayBuffer(1L to n.toLong: _*)
  def freshBuilder() = ArrayBuffer.newBuilder[Long]

  @Setup(Level.Trial)
  def initTrial(): Unit = {
    xs = fresh(size)
    ys = ArrayBuffer(1L to size.toLong / 2L: _*) ++ ArrayBuffer((size.toLong / 2L) to 1L by -1L: _*)
