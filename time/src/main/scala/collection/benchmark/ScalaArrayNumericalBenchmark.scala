...
class ScalaArrayNumericalBenchmark extends AbstractBenchmark {
  ...
  @Benchmark
  def sumOfSquares(bh: Blackhole): Unit = {
    var i = 0
    var ret = 0L
    while (i < v.length) {
      ret += v(i) * v(i)
