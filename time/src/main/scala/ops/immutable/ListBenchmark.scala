...
class ListBenchmark {
  ...
  val empty: List[Long] = List.empty[Long]
  var xs: List[Long] = _
  var ys: List[Long] = _
  var zs: List[Long] = _
  var zipped: List[(Long, Long)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = List(1L to n.toLong: _*)
  def freshBuilder() = List.newBuilder[Long]

  @Setup(Level.Trial)
  def initTrial(): Unit = {
    xs = fresh(size)
    ys = List(1L to size.toLong / 2L: _*) ::: List((size.toLong / 2L) to 1L by -1L: _*)
