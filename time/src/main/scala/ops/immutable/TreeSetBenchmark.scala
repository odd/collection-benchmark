...
class TreeSetBenchmark {
  ...
  val empty: TreeSet[Long] = TreeSet.empty[Long]
  var xs: TreeSet[Long] = _
  var ys: TreeSet[Long] = _
  var zs: TreeSet[Long] = _
  var zipped: TreeSet[(Long, Long)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = TreeSet(1L to n.toLong: _*)
  def freshBuilder() = TreeSet.newBuilder[Long]

  @Setup(Level.Trial)
  def initTrial(): Unit = {
    xs = fresh(size)
    ys = TreeSet(1L to size.toLong / 2L: _*) ++ TreeSet((size.toLong / 2L) to 1L by -1L: _*)
