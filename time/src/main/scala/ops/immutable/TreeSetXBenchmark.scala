...
import scala.collection.immutable.{TreeSetX, NumericRange}
...
class TreeSetXBenchmark {
  ...
  val empty: TreeSetX[Long] = TreeSetX.empty[Long]
  var xs: TreeSetX[Long] = _
  var ys: TreeSetX[Long] = _
  var zs: TreeSetX[Long] = _
  var zipped: TreeSetX[(Long, Long)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = TreeSetX(1L to n.toLong: _*)
  def freshBuilder() = TreeSetX.newBuilder[Long]

  @Setup(Level.Trial)
  def initTrial(): Unit = {
    xs = fresh(size)
    ys = TreeSetX(1L to size.toLong / 2L: _*) ++ TreeSetX((size.toLong / 2L) to 1L by -1L: _*)
