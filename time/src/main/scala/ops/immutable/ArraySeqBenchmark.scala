...
class ArraySeqBenchmark {
  ...
  val empty: ArraySeq[Long] = ArraySeq.empty[Long]
  var xs: ArraySeq[Long] = _
  var ys: ArraySeq[Long] = _
  var zs: ArraySeq[Long] = _
  var zipped: ArraySeq[(Long, Long)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = ArraySeq(1L to n.toLong: _*)
  def freshBuilder() = ArraySeq.newBuilder[Long]

  @Setup(Level.Trial)
  def initTrial(): Unit = {
    xs = fresh(size)
    ys = ArraySeq(1L to size.toLong / 2L: _*) ++ ArraySeq((size.toLong / 2L) to 1L by -1L: _*)
