...
import scala.collection.immutable.{NumericRange, VectorMap}
...
class VectorMapBenchmark {
  ...
  val empty: VectorMap[Long, Long] = VectorMap.empty[Long, Long]
  var xs: VectorMap[Long, Long] = _
  var zs: VectorMap[Long, Long] = _
  var zipped: VectorMap[Long, (Long, Long)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = VectorMap((1 to n).map(x => (x.toLong, x.toLong)): _*)
  def freshBuilder() = VectorMap.newBuilder[Long, Long]
