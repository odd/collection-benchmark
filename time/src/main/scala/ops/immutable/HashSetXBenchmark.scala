...
import scala.collection.immutable.{HashSetX, NumericRange}
...
class HashSetXBenchmark {
  ...
  val empty: HashSetX[Long] = HashSetX.empty[Long]
  var xs: HashSetX[Long] = _
  var ys: HashSetX[Long] = _
  var zs: HashSetX[Long] = _
  var zipped: HashSetX[(Long, Long)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = HashSetX(1L to n.toLong: _*)
  def freshBuilder() = HashSetX.newBuilder[Long]

  @Setup(Level.Trial)
  def initTrial(): Unit = {
    xs = fresh(size)
    ys = HashSetX(1L to size.toLong / 2L: _*) ++ HashSetX((size.toLong / 2L) to 1L by -1L: _*)
